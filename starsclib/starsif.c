...
int _stars_receive_lf(int sockn, char *message, struct timeval *timeout, int buflen){
	...
	while(1){
		...
		if(rt <= 0){
			...
			rt = recv(sockn, buf, 1, 0);
			if(rt <= 0){return(-1);}
			if(buf[0] == (char)0x0d){continue;}
			if(buf[0] == (char)0x0a){
				 message[rdp] = (char)0;
				 return(rdp);
			}
...
int stars_mainloop(int timeout){
	...
	char buf[32];
	int	 fh;      /*file handle*/
	char *mess;   /*pointer for read messge*/
	fd_set readfds,readfds_save;
	struct timeval cbtimeout;

	cbtimeout.tv_sec  = ((long)timeout)/1000L;
	cbtimeout.tv_usec = (((long)timeout) - (cbtimeout.tv_sec * 1000L)) * 1000L;

	if(stars_callback_list_count<1){return(-1);}

	while(1){
		FD_ZERO(&readfds_save);
		for(lp = 0; lp < stars_callback_list_count; lp++){
			FD_SET(stars_callback_list[lp]->handle, &readfds_save);
		}
		readfds = readfds_save;
		if(timeout == 0){
			rt=select(FD_SETSIZE, &readfds, NULL, NULL, NULL);
		}else{
			rt=select(FD_SETSIZE, &readfds, NULL, NULL, &cbtimeout);
		}
		if(rt <= 0){
			return(rt);
		}else{
			for(lp = 0; lp < stars_callback_list_count; lp++){
				fh = stars_callback_list[lp]->handle;
				mess = stars_callback_list[lp]->rcvmess;
				if(!FD_ISSET(fh, &readfds)){
					continue;
				}
				switch(stars_callback_list[lp]->cbmode){
					case STARS_CBMODE_STARS:
						if((rt2 = read(fh, buf, 1)) < 1){
							return(rt2);
						}
						if(buf[0] == (char)0x0d){break;}
						if(buf[0] == (char)0x0a){
							mess[stars_callback_list[lp]->rcvcnt] = (char)0;
							_stars_divide_message(stars_callback_list[lp]);
							if( (rt2=(stars_callback_list[lp]->cbfunc)
							     (stars_callback_list[lp])) < 0){return(rt2);}
							stars_callback_list[lp]->rcvcnt = 0;
							break;
						}
						mess[stars_callback_list[lp]->rcvcnt] = buf[0];
						if(stars_callback_list[lp]->rcvcnt < STARS_MESSAGE_MAX){
							stars_callback_list[lp]->rcvcnt++;
						}
						break;
					case STARS_CBMODE_CR:
						if((rt2 = read(fh, buf, 1)) < 1){
							return(rt2);
						}
						if(buf[0] == (char)0x0a){break;}
						if(buf[0] == (char)0x0d){
							mess[stars_callback_list[lp]->rcvcnt] = (char)0;
							if( (rt2=(stars_callback_list[lp]->cbfunc)
							     (stars_callback_list[lp])) < 0){return(rt2);}
					...
					case STARS_CBMODE_LF:
						...
						break;
					case STARS_CBMODE_DIRECT:
						if((rt2 = read(fh, buf, 1)) < 1){
							return(rt2);
						}
						mess[0] = buf[0];
						mess[1] = (char)0;
						stars_callback_list[lp]->rcvcnt = 1;
						if( (rt2=(stars_callback_list[lp]->cbfunc)
						     (stars_callback_list[lp])) < 0){return(rt2);}
						stars_callback_list[lp]->rcvcnt = 1;
						break;
					 case STARS_CBMODE_DETECT:
						if( (rt2=(stars_callback_list[lp]->cbfunc)
						     (stars_callback_list[lp])) < 0){return(rt2);}
						stars_callback_list[lp]->rcvcnt = 1;
						break;
