...
# 2016-04-18 Beta: Counter 8 -> 16,32 NCT08_PGM_VERSION:3
#----------------------------------------------------------------
# CVS $Date: 2016/04/18 12:00:00 $ $Revision: 2.0 $
#################################################################
use strict;
use Getopt::Long;
use stars;
...
use constant NCT08_PGM_VERSION    =>   3.0;    # pgm version
...
sub NCT08_getdevtype{
	my ($nodeno) = (shift);
	if(defined($::NCT08Devtypeno{$::NCT08Devtype{$nodeno}})){
		return($::NCT08Devtypeno{$::NCT08Devtype{$nodeno}});
...
sub NCT08actALM{
	...
	my $rt=NCT08act($nodeno,'ALM?',WAIT_MEMWRITE_ACT);
...
#	my($counterobit)=unpack("B8",pack("H2","$1"));
	my($counterobit)=unpack("B$chmax",pack("H4",split(//,"$1")));
...
sub CheckCounterInfo{
	...
	if($::NCT08Devtype{$nodeno} eq ''){
		...
		if($::NCT08Devtype{$nodeno}=~/(NCT08-02)/){
			...
			$::NCT08Devtype{$nodeno}=$1;
		}elsif($::NCT08Devtype{$nodeno}=~/\s(CT)(16)(-ER\S+)/){
			$::NCT08Devtype{$nodeno}="$1$2$3";
			$::NCT08NumberOfCounter{$nodeno}=32;
		}elsif($::NCT08Devtype{$nodeno}=~/\s(CT)(08|8|16|32|64)(\S+)/){
			#$::NCT08Devtype{$nodeno}=$1;
			$::NCT08Devtype{$nodeno}="$1$2$3";
			### 2016/4 Counter externsion 8,16,32,64 
		...
		}elsif($::NCT08Devtype{$nodeno}=~/^(CT)(08|8|16|32|64)(\S+)$/){
			### 2016/4 Counter externsion 8,16,32,64 
