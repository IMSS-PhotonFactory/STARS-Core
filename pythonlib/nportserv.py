...
"""
  nportserv python program
    ...
    History:
       0.2      1st version     2018.08.20    Yasuko Nagatani
       0.21     2nd version     2020.05.18    Yasuko Nagatani   Enabling the binary data exchange
"""
        ...
        try:
            if(isinstance(smsg, str)):
                if(isinstance(delimiter, str)):
                    ...
                    self.s.sendall(delimiter)
            else:
                self.s.sendall(smsg)
                if(isinstance(delimiter, str)):
                    self.s.sendall(delimiter.encode())
                else:
                    self.s.sendall(delimiter)
        except socket.error as e:
    ...
    def receive(self, timeout = '', delimiter=None, exceptionret = None):
        ...
        while True:
            ...
            try:
                ...
                rmsg = self.s.recv(self.TCP_BUFFER_SIZE)
                rmsg = rmsg.decode()
