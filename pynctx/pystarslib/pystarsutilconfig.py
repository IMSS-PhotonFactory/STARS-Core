...
class PyStarsUtilConfig():
    ...
    # Initialize:
    #----------------------------------------------------------------
    def __init__(self, configfilename, debug=False, defaults=None):
        default_section=''
        if(configfilename == ""):
        ...
        if(os.path.isfile(configfilename)==False):
            self._error="Configuration file not file. ('%s')" %(configfilename)
            self._debugprint("%s\n" %(self._error))
            return
        try:
            #cfg = ConfigParser(default_section='D')
        ...
        try:
            for i in range(len(p_seclist)):
                ...
                if(cfg.has_option(p_sec,paramname)==False):
                   ...
                   continue
                value = None
                if(sys.version_info.major>=3 and paramtype == bool):
                     value = cfg[p_sec].getboolean(paramname)
                else:
                    if(sys.version_info.major<3):
                        val = cfg.get(p_sec, paramname)
                    else:
                        val = cfg[p_sec].get(paramname)
                    if(val is None):
                        continue
                    elif(paramtype == bool):
                         value = distutils.util.strtobool(val)
