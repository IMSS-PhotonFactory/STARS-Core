...
#2005-09-28 Ver.1.5 Takashi Kosuge
#  act blocks commands and inserts it into queue until reply will be received.
...
#Usage: stars->Mainloop([\&sub, timeout|\timeout [, FlgDoEvents]]);
#       Scan once when "FlgDoEvents" is 1. It is used for "DoEvents" function.
#Changed, timeout can be reference. 2002-02-22
#Changed, supports blocking input of "act" 2005-09-29
sub Mainloop{
	my $dummy=shift;
	my $handler=shift;
	my $timeoutval=shift;
	my $FlgDoEvents=shift;
	my $timeout;
	my $fintimeout;
	my $ready;
...
#Do input queue of STARS (with time interval)
			for $s (keys(%stars::cbmode)){
				if($stars::cbmode{$s} ne 'Stars'){next;}
				while($buf=shift(@{$stars::cbqueue{$s}})){
			...
			if($stars::cbmode{$s} eq 'Detect'){
				...
				while($stars::cbbuf{$s} =~ s/([^\r\n]*)\r*\n//){
					...
					if($stars::cbmode{$s} eq 'Stars'){
						push(@{$stars::cbqueue{$s}}, $buf);
#Do input queue of STARS
						while($buf=shift(@{$stars::cbqueue{$s}})){
							unless($buf =~ s/^([a-zA-Z_0-9.\-]+)>([a-zA-Z_0-9.\-]+)\s*//){next;}
							$stars::cbhandler{$s}->("$1","$2","$buf");
						}
...
#			$stars::cbhandler{$this->{fh}}->($from, $to, $buf);
			push(@{$stars::cbqueue{$this->{fh}}}, "$from>$to $buf"); 
		}
	}
