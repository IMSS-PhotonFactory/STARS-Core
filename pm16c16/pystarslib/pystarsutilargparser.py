...
class PyStarsUtilArgParser():
    ...
    # Initialize:
    #----------------------------------------------------------------
    def __init__(self, numberOfStarsServer=1, numberOfDeviceServer=1, useConfigFile=True, useLogOutput=False, useRawEnable=False, useDebugLevel=False, useLogOutputLevel=False):
        self._parser = None
        self._error = ''
        ...
        self._optdict['useDebugLevel']        = useDebugLevel;
        self._optdict['useLogOutputLevel']    = useLogOutputLevel;
        self._optdict['useRawEnable']    = useRawEnable;
    ...
    def generate_baseparser(self,prog=None,version=None,usage=None,description=None,epilog=None):
        ...
        if(num>0):
            ...
            if(self._optdict['useRawEnable']):
                pparser.add_argument('--rawenable', action='store_true', default=False, help='enable the stars command to send the device native command to device if this flag is set (default: False)')
            #Add 2021.10.1 
            for i in range(1,num):
                no = str(i+1)
                ...
                #Add 2021.10.1 
                if(self._optdict['useRawEnable']):
                    pparser.add_argument('--rawenable'+no, action='store_true', default=False, help='enable the stars command to send the device native command to device '+no+' if this flag is set (default: False)')
...
if __name__ == '__main__':
    ...
    parser=optIO.generate_baseparser(prog='TEST',version=1.0)
    parser.add_argument('args', nargs='*', help='Command line arguments.')
    args=parser.parse_args()
    print("Program args"+str(args.args))
    debug=args.debug
